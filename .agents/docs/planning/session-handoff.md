...
**A type an application can encounter lives at the root; a thing only an integrator constructs lives on `/config`.** `ParagraphMeasurement` is a root result type; renderer schema and Codec builders live on the shared config entry. There is no second public core runtime, and `entry-point-boundaries.test.mjs` enforces that split.
...
- Renderer-neutral integration helpers remain available on `/config`, while TSL shaders remain available on `/tsl/*`. The retired `/core` surface is not a second engine-driving API.
